...
const MyJob = () => {
  ...
  const { isPending, error, data } = useQuery({
    ...
    queryFn: () =>
      axios
        .get(`http://localhost:5000/getmyjob/${user?.email}`, {withCredentials: true})
        .then((res) => res.data),
  });
                ...
                <tr key={dataAddedMy._id}>
                  ...
                  <td>
                    <Link to={`/updatejob/${dataAddedMy?._id}`}>
                      <button className="px-3 py-2 rounded-lg bg-green-300 text-white">
                        Update
                      </button>
                    </Link>
                  </td>
                  <td>
                    <button
                      onClick={() => handleDeleteBtn(dataAddedMy?._id)}
